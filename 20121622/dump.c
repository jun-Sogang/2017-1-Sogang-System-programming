  ...
  if (*start >= maxSize) {
    ...
    while (1) {
      if (*start <= end && *start < maxSize) {           //Over Memory?
        printAll(*start, end);      //Print one line
        *start += 16 - *start % 16;     //Make start to first of next line address
      } else {
...
void printAll(int nowAddress, int end) {
...
void printData(int nowAddress, int addressStart, int end) {
...
void printASCII(int addressStart) {
